...
class TestPluginMetadata:
    ...
    def test_to_dict(self):
        from fluid_build.cli.plugins import PluginMetadata, PluginType
        m = PluginMetadata(
            name="p", version="2.0", description="d", author="a",
            homepage="http://ex.com", documentation="http://docs.com",
            plugin_type=PluginType.PROVIDER, fluid_version_min="3.0.0",
            fluid_version_max="4.0.0", dependencies=["dep1"], permissions=["read"]
        )
        d = m.to_dict()
        assert d["name"] == "p"
...
class TestPluginManager:
    ...
    def test_list_plugins_filter_type(self, tmp_path):
        from fluid_build.cli.plugins import (
            PluginManager, PluginMetadata, InstalledPlugin, PluginType, PluginStatus
        )
        pm = PluginManager(plugin_dir=tmp_path)
        m1 = PluginMetadata(name="a", version="1.0", description="d", author="a", plugin_type=PluginType.COMMAND)
        m2 = PluginMetadata(name="b", version="1.0", description="d", author="a", plugin_type=PluginType.PROVIDER)
    ...
    def test_get_plugin(self, tmp_path):
        from fluid_build.cli.plugins import (
            PluginManager, PluginMetadata, InstalledPlugin, PluginStatus
        )
        pm = PluginManager(plugin_dir=tmp_path)
    ...
    def test_register_plugin_command(self, tmp_path):
        from fluid_build.cli.plugins import (
            PluginManager, PluginMetadata, InstalledPlugin, PluginType, PluginStatus, CommandPlugin
        )
        pm = PluginManager(plugin_dir=tmp_path)
        m = PluginMetadata(name="cmd1", version="1.0", description="d", author="a", plugin_type=PluginType.COMMAND)
        mock_instance = MagicMock(spec=CommandPlugin)
        p = InstalledPlugin(metadata=m, path=tmp_path/"cmd1", instance=mock_instance, status=PluginStatus.ACTIVE)
        pm._register_plugin(p)
        assert "cmd1" in pm.command_plugins

    def test_register_plugin_provider(self, tmp_path):
        from fluid_build.cli.plugins import (
            PluginManager, PluginMetadata, InstalledPlugin, PluginType, PluginStatus, ProviderPlugin
        )
        pm = PluginManager(plugin_dir=tmp_path)
        m = PluginMetadata(name="prov1", version="1.0", description="d", author="a", plugin_type=PluginType.PROVIDER)
        mock_instance = MagicMock(spec=ProviderPlugin)
        p = InstalledPlugin(metadata=m, path=tmp_path/"prov1", instance=mock_instance, status=PluginStatus.ACTIVE)
        pm._register_plugin(p)
        assert "prov1" in pm.provider_plugins

    def test_register_plugin_validator(self, tmp_path):
        from fluid_build.cli.plugins import (
            PluginManager, PluginMetadata, InstalledPlugin, PluginType, PluginStatus, ValidatorPlugin
        )
        pm = PluginManager(plugin_dir=tmp_path)
        m = PluginMetadata(name="val1", version="1.0", description="d", author="a", plugin_type=PluginType.VALIDATOR)
        mock_instance = MagicMock(spec=ValidatorPlugin)
        p = InstalledPlugin(metadata=m, path=tmp_path/"val1", instance=mock_instance, status=PluginStatus.ACTIVE)
        pm._register_plugin(p)
    ...
    def test_disable_plugin_active(self, tmp_path):
        ...
        mock_inst = MagicMock()
        p = InstalledPlugin(metadata=m, path=tmp_path/"p", instance=mock_inst, status=PluginStatus.ACTIVE)
    ...
    def test_enable_with_instance(self, tmp_path):
        ...
        p = InstalledPlugin(metadata=m, path=tmp_path/"p", instance=inst, status=PluginStatus.INACTIVE, config={"k": "v"})
        pm.installed_plugins["p"] = p
        assert pm.enable_plugin("p") is True
    ...
    def test_enable_instance_fails(self, tmp_path):
        ...
        p = InstalledPlugin(metadata=m, path=tmp_path/"p", instance=inst, status=PluginStatus.INACTIVE)
        pm.installed_plugins["p"] = p
...
class TestPluginsRun:
    ...
    def test_run_info(self, mock_gpm):
        from fluid_build.cli.plugins import run, PluginMetadata, InstalledPlugin, PluginStatus
        pm = MagicMock()
        m = PluginMetadata(name="p", version="1.0", description="d", author="a")
        pm.get_plugin.return_value = InstalledPlugin(metadata=m, path=Path("/tmp/p"), status=PluginStatus.ACTIVE)
        mock_gpm.return_value = pm
    ...
    def test_run_create(self, mock_gpm, tmp_path):
        from fluid_build.cli.plugins import run
        mock_gpm.return_value = MagicMock()
        args = self._make_args("create", name=str(tmp_path / "testplugin"), type="command", author="tester")
...
class TestPluginTemplates:
    ...
    def test_basic_template(self):
        from fluid_build.cli.plugins import _get_basic_plugin_template, PluginType
...
class TestHandleCreatePlugin:
    def test_create_success(self, tmp_path, monkeypatch):
        from fluid_build.cli.plugins import handle_create_plugin, PluginManager
