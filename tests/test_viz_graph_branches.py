...
class TestGraphConfig:
    ...
    def test_custom_theme_missing(self, tmp_path):
        ...
        contract.write_text("name: test")
        with pytest.raises(FileNotFoundError):
            GraphConfig(contract_path=str(contract), theme="custom", custom_theme_path="/missing/theme.json")
    ...
    def test_custom_theme_valid(self, tmp_path):
        ...
        cfg = GraphConfig(contract_path=str(contract), theme="custom", custom_theme_path=str(theme_file))
    ...
    def test_all_options(self, tmp_path):
        ...
        contract.write_text("name: test")
        cfg = GraphConfig(
            contract_path=str(contract), output_path="out.svg",
            format="svg", theme="light", rankdir="TB",
            title="My Graph", show_legend=True,
            collapse_consumes=True, collapse_exposes=True,
            show_metadata=False, show_descriptions=True,
            max_label_length=30, open_when_done=True,
...
class TestRunFunction:
    ...
    def test_run_dot_format(self, mock_load, mock_write, tmp_path):
        ...
        mock_load.return_value = {"name": "test", "id": "test-id", "version": "1.0", "exposes": [], "consumes": []}
        output = tmp_path / "output" / "out.dot"
        args = argparse.Namespace(
            contract=str(contract_file), output=str(output),
            format="dot", env=None, plan=None, theme="dark",
            rankdir="LR", title=None, legend=False,
            collapse_consumes=False, collapse_exposes=False,
            show_metadata=True, show_descriptions=False,
            max_label_length=50, open=False, force=True,
            quiet=True, custom_theme=None, graphviz_args=None
        )
        result = run(args, logging.getLogger("test"))
        assert result == 0

    @patch("fluid_build.cli.viz_graph.load_contract_with_overlay", side_effect=FileNotFoundError("nope"))
    def test_run_missing_contract(self, mock_load, tmp_path):
        from fluid_build.cli.viz_graph import run
        from fluid_build.cli._common import CLIError
        args = argparse.Namespace(
            contract="/nonexistent", output="out.dot",
            format="dot", env=None, plan=None, theme="dark",
            rankdir="LR", title=None, legend=False,
            collapse_consumes=False, collapse_exposes=False,
            show_metadata=True, show_descriptions=False,
