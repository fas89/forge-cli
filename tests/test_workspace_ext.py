...
class TestTeamMemberToDict:
    def test_basic(self):
        ...
        tm = TeamMember(id="u1", name="Alice", email="a@b.com", role=WorkspaceRole.DEVELOPER, joined_at=now)
        d = tm.to_dict()
        assert d["id"] == "u1"
        assert d["name"] == "Alice"
    ...
    def test_with_last_active_and_permissions(self):
        ...
        tm = TeamMember(id="u2", name="Bob", email="b@b.com", role=WorkspaceRole.ADMIN,
                        joined_at=now, last_active=active, permissions={"read", "write"})
        d = tm.to_dict()
...
class TestContractVersionToDict:
    def test_basic(self):
        ...
        cv = ContractVersion(id="v1", contract_path="/a/b.yaml", version="1.0.0",
                             author="alice", status=ContractStatus.APPROVED,
                             created_at=now, message="Initial release")
        d = cv.to_dict()
        assert d["id"] == "v1"
    ...
    def test_with_changes_and_reviewers(self):
        cv = ContractVersion(id="v2", contract_path="/x.yaml", version="2.0.0",
                             author="bob", status=ContractStatus.IN_REVIEW,
                             created_at=datetime(2024, 3, 1), message="Update",
                             changes=["added expose"], reviewers=["alice"],
                             approvals=["alice"])
        d = cv.to_dict()
...
class TestChangeRequestToDict:
    def test_basic(self):
        ...
        cr = ChangeRequest(id="cr1", title="Add table", description="Add new expose",
                           author="alice", target_contract="/c.yaml",
    ...
    def test_with_comments(self):
        now = datetime(2024, 5, 1)
        cr = ChangeRequest(id="cr2", title="Fix", description="Fix schema",
                           author="bob", target_contract="/c.yaml",
                           status=ChangeRequestStatus.MERGED,
                           created_at=now, updated_at=now,
                           changes={}, comments=[{"user": "alice", "text": "LGTM"}])
        d = cr.to_dict()
...
class TestWorkspaceConfigToDict:
    def test_basic(self):
        now = datetime(2024, 1, 1)
        wc = WorkspaceConfig(name="ws1", description="Test workspace",
                             owner="alice", created_at=now)
        d = wc.to_dict()
        assert d["name"] == "ws1"
    ...
    def test_with_settings(self):
        now = datetime(2024, 1, 1)
        wc = WorkspaceConfig(name="ws2", description="With settings",
                             owner="bob", created_at=now,
