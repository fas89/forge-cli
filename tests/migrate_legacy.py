...
def main():
    ...
    if not new_tests_dir.exists():
        if RICH_AVAILABLE and console:
            console.print("[red]❌ New test structure not found. Please set up the consolidated tests first.[/red]")
        else:
    ...
    if not legacy_scripts:
        if RICH_AVAILABLE and console:
            console.print("[green]✅ No legacy test scripts found. Migration already complete![/green]")
        else:
    ...
    if choice == "1":
        ...
        # Remove scripts
        if RICH_AVAILABLE and console:
            if Confirm.ask("[red]⚠️ Are you sure you want to permanently delete these scripts?[/red]"):
                for script in legacy_scripts:
                    script.unlink()
        ...
        # Keep scripts
        if RICH_AVAILABLE and console:
            console.print("[yellow]⚠️ Legacy scripts will be kept. This may cause confusion.[/yellow]")
    ...
    if RICH_AVAILABLE and console:
        ...
        console.print(f"   • New consolidated test structure: [green]✅ Available[/green]")
        console.print(f"   • Legacy scripts: [{'green' if choice in ['1', '2'] else 'yellow'}]{'✅ Handled' if choice in ['1', '2'] else '⚠️ Still present'}[/{'green' if choice in ['1', '2'] else 'yellow'}]")
        console.print(f"   • Admin command integration: [green]✅ Ready[/green]")
