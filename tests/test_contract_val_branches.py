...
class TestValidationIssue:
    ...
    def test_str_full(self):
        from fluid_build.cli.contract_validation import ValidationIssue
        issue = ValidationIssue(
            severity="warning", category="binding", message="check",
            path="exposes[0]", expected="int", actual="str",
            suggestion="Fix type", documentation_url="http://docs.example.com"
        )
        s = str(issue)
    ...
    def test_str_with_path(self):
        from fluid_build.cli.contract_validation import ValidationIssue
        issue = ValidationIssue(severity="info", category="metadata", message="ok", path="root.field")
        s = str(issue)
...
class TestValidationReport:
    def _make_report(self):
        ...
        return ValidationReport(
            contract_path="test.yaml", contract_id="test-dp",
            contract_version="1.0.0", validation_time=datetime.now(),
            duration=0.5
        )
    ...
    def test_add_issue_with_suggestion(self):
        report = self._make_report()
        report.add_issue("error", "binding", "msg", "p",
                         expected="A", actual="B", suggestion="Fix it",
...
class TestContractValidator:
    ...
    @patch("fluid_build.cli.contract_validation.load_contract_with_overlay", side_effect=FileNotFoundError("nope"))
    def test_validate_contract_not_found(self, mock_load):
        from fluid_build.cli.contract_validation import ContractValidator
        v = ContractValidator(contract_path=Path("missing.yaml"), use_cache=False, track_history=False)
...
class TestProviderDetection:
    ...
    def test_detect_gcp_provider(self, mock_schema, mock_load):
        ...
        mock_load.return_value = {
            "id": "test", "version": "1.0",
            "exposes": [{"id": "dp", "type": "table", "binding": {"platform": "gcp", "location": {"properties": {"project": "myproj", "dataset": "ds", "table": "t"}, "format": "bigquery"}}, "schema": []}]
        }
        mock_result = MagicMock(is_valid=True, errors=[], warnings=[])
        mock_schema.return_value.validate_contract.return_value = mock_result
        v = ContractValidator(contract_path=Path("test.yaml"), use_cache=False, track_history=False, check_data=False)
        report = v.validate()
    ...
    def test_detect_aws_provider(self, mock_schema, mock_load):
        ...
        mock_load.return_value = {
            "id": "test", "version": "1.0",
            "exposes": [{"id": "dp", "type": "table", "binding": {"platform": "aws", "location": {}}, "schema": []}]
        }
        mock_result = MagicMock(is_valid=True, errors=[], warnings=[])
        mock_schema.return_value.validate_contract.return_value = mock_result
        v = ContractValidator(contract_path=Path("test.yaml"), use_cache=False, track_history=False, check_data=False)
    ...
    def test_detect_unknown_provider(self, mock_schema, mock_load):
        ...
        mock_load.return_value = {
            "id": "test", "version": "1.0",
            "exposes": [{"id": "dp", "type": "table", "binding": {"platform": "unknown_provider", "location": {}}, "schema": []}]
        }
        mock_result = MagicMock(is_valid=True, errors=[], warnings=[])
        mock_schema.return_value.validate_contract.return_value = mock_result
        v = ContractValidator(contract_path=Path("test.yaml"), use_cache=False, track_history=False, check_data=False)
        report = v.validate()
        errors = [i for i in report.issues if i.severity == 'error' and 'Unknown provider' in i.message]
        assert len(errors) >= 1
    ...
    def test_detect_databricks_provider(self, mock_schema, mock_load):
        ...
        mock_load.return_value = {
            "id": "test", "version": "1.0",
            "exposes": [{"id": "dp", "type": "table", "binding": {"platform": "databricks", "location": {}}, "schema": []}]
        }
        mock_result = MagicMock(is_valid=True, errors=[], warnings=[])
        mock_schema.return_value.validate_contract.return_value = mock_result
        v = ContractValidator(contract_path=Path("test.yaml"), use_cache=False, track_history=False, check_data=False)
...
class TestValidateBinding:
    ...
    def test_missing_location(self, mock_schema, mock_load):
        ...
        mock_load.return_value = {
            "id": "test", "version": "1.0",
            "exposes": [{"id": "dp", "type": "table", "binding": {"platform": "gcp"}, "schema": []}]
        }
        mock_result = MagicMock(is_valid=True, errors=[], warnings=[])
        mock_schema.return_value.validate_contract.return_value = mock_result
        v = ContractValidator(contract_path=Path("test.yaml"), use_cache=False, track_history=False, check_data=False)
    ...
    def test_gcp_missing_props(self, mock_schema, mock_load):
        ...
        mock_load.return_value = {
            "id": "test", "version": "1.0",
            "exposes": [{"id": "dp", "type": "table",
                         "binding": {"platform": "gcp", "location": {"format": "bigquery", "properties": {}}},
                         "schema": []}]
        }
        mock_result = MagicMock(is_valid=True, errors=[], warnings=[])
        mock_schema.return_value.validate_contract.return_value = mock_result
...
class TestContractValidationCLI:
    ...
    def test_run_valid(self, mock_validator_cls, tmp_path):
        from fluid_build.cli.contract_validation import run, ValidationReport
        contract_file = tmp_path / "test.yaml"
        contract_file.write_text("name: test")
        mock_report = ValidationReport(
            contract_path=str(contract_file), contract_id="test",
            contract_version="1.0", validation_time=datetime.now(),
            duration=0.1
        )
        mock_validator_cls.return_value.validate.return_value = mock_report
        args = argparse.Namespace(
            contract=str(contract_file), env=None, provider=None,
            project=None, region=None, strict=False,
            no_data=False, output_file=None, output_format="text",
            verbose=False, server=None,
            cache=True, cache_ttl=3600, cache_clear=False,
    ...
    def test_run_json_output(self, mock_validator_cls, tmp_path):
        ...
        args = argparse.Namespace(
            contract=str(contract_file), env=None, provider=None,
            project=None, region=None, strict=False,
            no_data=False, output_file=str(output_file), output_format="json",
            verbose=False, server=None,
            cache=True, cache_ttl=3600, cache_clear=False,
            track_history=True, check_drift=False
        )
    ...
    def test_run_missing_file(self):
        ...
        args = argparse.Namespace(
            contract="/nonexistent/contract.yaml", env=None,
    ...
    def test_run_strict_with_warnings(self, mock_validator_cls, tmp_path):
        ...
        args = argparse.Namespace(
            contract=str(contract_file), env=None, provider=None,
            project=None, region=None, strict=True,
            no_data=False, output_file=None, output_format="text",
            verbose=False, server=None,
            cache=True, cache_ttl=3600, cache_clear=False,
            track_history=True, check_drift=False
