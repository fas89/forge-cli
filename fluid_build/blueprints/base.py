...
class Blueprint:
    ...
    def _load_metadata(self):
        ...
        if yaml is None:
            raise ImportError(
                "PyYAML is required for blueprint support. Install with: pip install PyYAML"
            )
        ...
        self.metadata.dependencies = [BlueprintDependency(**dep) for dep in deps_data]
    ...
    def generate_project(self, target_dir: Path, customizations: Dict[str, Any] = None) -> None:
        ...
        copytree(
            self.path, target_dir, ignore=ignore_patterns("blueprint.yaml", "*.pyc", "__pycache__")
        )
