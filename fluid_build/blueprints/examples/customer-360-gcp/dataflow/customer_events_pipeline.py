...
class ParseCustomerEvent(beam.DoFn):
    ...
    def _transform_event(self, event_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        processed_event = {
            ...
            "revenue_amount": (
                float(event_data.get("revenue_amount", 0))
                if event_data.get("revenue_amount")
                else None
            ),
            # Derived fields
...
class CustomerEventPipeline:
    ...
    def run(self):
        ...
        with beam.Pipeline(options=self.options) as pipeline:
            ...
            # Read from Pub/Sub
            events = pipeline | "Read from Pub/Sub" >> ReadFromPubSub(
    ...
    def parse_arguments(self):
        ...
        parser.add_argument(
            "--input_subscription", required=True, help="Pub/Sub subscription to read from"
        )
        ...
        parser.add_argument("--backup_bucket", help="GCS bucket for backup storage (optional)")
