...
def register(subparsers: argparse._SubParsersAction) -> None:
    ...
    parser.add_argument(
        "contract",
        help="Path to FLUID contract file (contract.fluid.yaml)"
    )
    
    parser.add_argument(
        "--env",
        help="Environment overlay (dev, staging, prod)"
    )
    ...
    parser.add_argument(
        "--output",
        "-o",
        help="Output file for policy report (JSON)"
    )
    ...
    parser.add_argument(
        "--show-passed",
        action="store_true",
        help="Show passed checks (default: only violations)"
    )
        ...
        if RICH_AVAILABLE and args.format == "rich":
            ...
            with Progress(
                ...
                if not contract_path.exists():
                    raise CLIError(1, "file_not_found", context={"message": f"Contract not found: {args.contract}"})
                ...
                time.sleep(0.2)
                progress.update(task, description=f"✨ Analysis complete: {result.calculate_score()}/100")
                time.sleep(0.3)
        else:
...
def output_rich(
    ...
    header.add_row("📊 Policy Score:", f"[{score_color} bold]{score_emoji} {score}/100[/{score_color} bold] [dim]({grade})[/dim]")
    ...
    category_meta = {
        PolicyCategory.SENSITIVITY: {"icon": "🔒", "name": "Data Sensitivity & Privacy", "color": "magenta"},
        PolicyCategory.ACCESS_CONTROL: {"icon": "🛡️", "name": "Access Control & Authorization", "color": "blue"},
        PolicyCategory.DATA_QUALITY: {"icon": "📊", "name": "Data Quality Standards", "color": "cyan"},
        PolicyCategory.LIFECYCLE: {"icon": "🔄", "name": "Lifecycle Management", "color": "yellow"},
        PolicyCategory.SCHEMA_EVOLUTION: {"icon": "⚡", "name": "Schema Evolution Control", "color": "green"},
    }
    ...
    for category, violations in categories.items():
        ...
        if violations or show_passed:
            ...
            if violations:
                # Group by severity
                for severity in [PolicySeverity.CRITICAL, PolicySeverity.ERROR, PolicySeverity.WARNING, PolicySeverity.INFO]:
