...
def register(subparsers: argparse._SubParsersAction):
    ...
    p = subparsers.add_parser(
        COMMAND,
        help="🔧 Manage and deploy FLUID blueprints - complete data product templates"
    )
    ...
    list_parser.add_argument('--category', choices=[c.value for c in BlueprintCategory], 
                           help='Filter by category')
    list_parser.add_argument('--complexity', choices=[c.value for c in BlueprintComplexity],
    ...
    create_parser.add_argument('--quickstart', '-q', action='store_true', 
    ...
    validate_parser.add_argument('name', nargs='?', help='Blueprint name to validate (all if not specified)')
...
def list_blueprints(args, logger: logging.Logger) -> int:
    ...
    for blueprint in blueprints:
        ...
        if args.verbose:
            ...
            logger.info(f"   Category: {metadata.category.value} | Complexity: {metadata.complexity.value}")
        
        logger.info("")
    
    if not args.verbose:
        logger.info("💡 Use --verbose for more details or 'fluid blueprint describe <name>' for full info")
...
def search_blueprints(args, logger: logging.Logger) -> int:
    ...
    for blueprint in blueprints:
        ...
        logger.info(f"   Tags: {', '.join(metadata.tags[:3])}{'...' if len(metadata.tags) > 3 else ''}")
        logger.info("")
