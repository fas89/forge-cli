...
• Runtime dependencies
    ...
    p.add_argument("--out-dir", default="runtime/diag", help="Output directory for diagnostic files")
    p.add_argument("--features-only", action="store_true", help="Only check FLUID feature availability (skip infrastructure)")
...
def run(args, logger: logging.Logger) -> int:
    ...
    try:
        ...
        info(logger, "doctor_script_missing", script=str(script_path), note="Skipping infrastructure checks")
    ...
    try:
        ...
        raise CLIError(1, "doctor_output_dir_failed", context={"out_dir": args.out_dir, "error": str(e)})
    ...
    if getattr(args, "provider", None):
        ...
        if provider != args.provider:
            secure_logger.log_safe("warning", f"Provider name sanitized: {args.provider} -> {provider}")
...
def _check_fluid_features() -> Tuple[bool, List[Dict[str, any]]]:
    ...
    try:
        from fluid_build.providers.gcp.provider_action_handler import GCPProviderActionHandler
        checks.append({
    ...
    try:
        from fluid_build.providers.aws.actions import s3, glue, iam  # noqa: F401
        checks.append({
...
def _print_feature_checks(checks: List[Dict[str, any]], verbose: bool = False):
    ...
    if RICH_AVAILABLE:
        ...
        if ok_count == total:
            console.print(Panel(
                "[green]✅ All critical features available![/green]",
                border_style="green"
            ))
