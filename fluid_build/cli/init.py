...
from ._logging import info, error
from ._common import CLIError
from fluid_build.cli.console import cprint, error as console_error, success, warning
...
def register(subparsers: argparse._SubParsersAction):
    ...
    mode_group.add_argument(
        "--wizard",
        action="store_true",
        help="🎨 Interactive guided setup (full control)"
    )
    mode_group.add_argument(
        "--blank",
        action="store_true",
        help="🔧 Empty project skeleton (power users)"
    )
    mode_group.add_argument(
        "--template",
        metavar="NAME",
...
def detect_mode(args, logger: logging.Logger) -> Optional[str]:
    ...
    if (cwd / "dbt_project.yml").exists():
        if RICH_AVAILABLE:
            ...
            console.print("Suggestion: Use [bold]fluid init --scan[/bold] to import your dbt models")
        return "scan"
    
    if (cwd / "main.tf").exists() or list(cwd.glob("*.tf")):
        if RICH_AVAILABLE:
            console.print("🔍 [cyan]Detected Terraform project[/cyan]")
            console.print("Suggestion: Use [bold]fluid init --scan[/bold] to import your infrastructure")
        return "scan"
...
def generate_dag_for_project(project_dir: Path, contract: dict, logger, console, template: str = None) -> bool:
    """
    ...
    try:
        ...
        if result.returncode != 0:
            ...
            logger.warning(f"generate-airflow command not available, creating basic DAG template")
...
def quickstart_mode(args, logger: logging.Logger) -> int:
    ...
    if project_dir.exists() and any(project_dir.iterdir()):
        if RICH_AVAILABLE:
            console.print(f"[red]❌ Directory '{project_name}' already exists and is not empty[/red]")
        else:
            ...
            try:
                ...
                if contract_path.exists():
                    ...
                    if should_generate_dag(contract, template):
                        has_dag = generate_dag_for_project(project_dir, contract, logger, console if RICH_AVAILABLE else None, template)
            except Exception as e:
                logger.warning(f"Failed to generate DAG: {e}")
...
def run_local_pipeline(project_dir: Path, logger: logging.Logger):
    ...
    try:
        ...
        console.print(f"  [cyan]$ fluid apply contract.fluid.yaml --provider local[/cyan]")


def show_success_message(project_dir: Path, provider: str, logger: logging.Logger, has_dag: bool = False):
    ...
    if not RICH_AVAILABLE:
        ...
        cprint(f"  $ fluid validate contract.fluid.yaml")
        return
    ...
    if provider == "local":
        ...
        if db_file.exists():
            console.print(f"  💾 Local database: .fluid/db.duckdb")
    ...
    console.print(f"  [bold yellow]2.[/bold yellow]  [cyan]fluid validate contract.fluid.yaml[/cyan]   [dim]# check the contract[/dim]")

    if provider == "local":
        console.print(f"  [bold yellow]3.[/bold yellow]  [cyan]fluid apply contract.fluid.yaml --yes[/cyan]  [dim]# run the pipeline[/dim]")
    else:
        console.print(f"  [bold yellow]3.[/bold yellow]  [cyan]fluid plan contract.fluid.yaml --provider {provider}[/cyan]")
...
def generate_cicd(project_dir: Path, logger: logging.Logger):
    ...
    if RICH_AVAILABLE:
        console.print("\n" + "─" * 70)
        want_cicd = Confirm.ask(
            "🔧 Generate CI/CD pipeline? (Recommended for teams)",
            default=True
        )
...
class DbtDetector(ProjectDetector):
    ...
    def _parse_model(self, sql_file: Path, logger: logging.Logger) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if matches:
                ...
                col_names = [c.strip().split()[-1].split('.')[-1] 
                            for c in col_text.split(',') if c.strip() and c.strip() != '*']
    ...
    def _detect_pii(self, models: List[Dict]) -> List[Dict[str, Any]]:
        ...
        pii_keywords = {
            ...
            "credit_card": {"patterns": ["credit_card", "cc_number", "card_num"], "confidence": 0.95},
            "address": {"patterns": ["address", "street", "zip", "postal"], "confidence": 0.70},
            "name": {"patterns": ["first_name", "last_name", "full_name", "customer_name"], "confidence": 0.60},
...
def show_scan_results(results: Dict[str, Any]):
    ...
    if project_type == "dbt":
        console.print(f"Project Name: [bold]{results['metadata'].get('project_name', 'N/A')}[/bold]")
...
def generate_contracts_from_scan(results: Dict[str, Any], provider: str, logger: logging.Logger) -> List[Dict]:
    ...
    if project_type == "dbt":
        ...
        contract["binding"] = {
            "provider": target_platform if target_platform in ["gcp", "snowflake", "aws"] else "local"
        }
        ...
        contract = {
            ...
            "produces": [],
            "binding": {
                "provider": results["metadata"].get("target_platform", "local")
            }
        }
        contracts.append(contract)
...
def apply_governance_policies(contracts: List[Dict], results: Dict[str, Any], logger: logging.Logger) -> List[Dict]:
        ...
        for produce in contract.get("produces", []):
            ...
            if model_name in by_model:
                ...
                for finding in by_model[model_name][:3]:  # Limit to 3
                    console.print(f"  • {finding['column']} ([{finding['type']}], {finding['confidence']:.0%} confidence)")
...
def show_migration_summary(contracts: List[Dict], results: Dict[str, Any], logger: logging.Logger):
    ...
    for contract in contracts:
        ...
        if contract.get("sovereignty"):
            console.print(f"   Governance: [yellow]GDPR controls enabled[/yellow]")
