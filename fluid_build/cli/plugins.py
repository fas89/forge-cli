...
class PluginManager:
        ...
        for plugin_path in self.plugin_dir.iterdir():
            if plugin_path.is_dir() and (plugin_path / 'plugin.json').exists():
                try:
                    ...
                    if self.console:
                        self.console.print(f"[red]Failed to load plugin {plugin_path.name}: {e}[/red]")
    ...
    def install_plugin(self, source: Union[str, Path], force: bool = False) -> bool:
        ...
        try:
            ...
            if not metadata_file.exists():
                if self.console:
                    self.console.print(f"[red]Invalid plugin: missing plugin.json[/red]")
                return False
            ...
            if plugin_name in self.installed_plugins and not force:
                if self.console:
                    self.console.print(f"[yellow]Plugin {plugin_name} already installed. Use --force to overwrite.[/yellow]")
                return False
    ...
    def execute_plugin_command(self, plugin_name: str, command: str, args: argparse.Namespace, logger: logging.Logger) -> int:
...
def register(subparsers: argparse._SubParsersAction):
    ...
    p = subparsers.add_parser(
        COMMAND,
        help="🔌 Manage FLUID plugins and extensions"
    )
    ...
    list_parser.add_argument('--type', choices=[t.value for t in PluginType], help='Filter by plugin type')
    list_parser.add_argument('--status', choices=[s.value for s in PluginStatus], help='Filter by status')
    ...
    create_parser.add_argument('--type', choices=[t.value for t in PluginType], default='command', help='Plugin type')
...
def run(args, logger: logging.Logger) -> int:
    ...
    try:
        ...
        if args.plugin_action == 'list':
            ...
            if RICH_AVAILABLE:
                console = Console()
                console.print("[red]❌ Unknown plugin action. Use 'fluid plugins --help' for available options.[/red]")
            return 1
...
def handle_list_plugins(args, plugin_manager: PluginManager, logger: logging.Logger) -> int:
    ...
    for plugin in plugins:
        ...
        table.add_row(
            ...
            plugin.metadata.description[:50] + "..." if len(plugin.metadata.description) > 50 else plugin.metadata.description
        )
...
[bold]Permissions:[/bold] {', '.join(plugin.metadata.permissions) if plugin.metadata.permissions else 'None'}
    ...
    console.print(Panel(info_text.strip(), title=f"Plugin: {plugin.metadata.name}", border_style="blue"))
    return 0
