...
@click.option(
    "--quality/--no-quality",
    default=True,
    help="Include quality checks (default: true)"
)
@click.option(
    "--sla/--no-sla",
    default=True,
    help="Include SLA properties (default: true)"
)
def export_command(
    contract: str,
    output: Optional[str],
...
def info_command():
    ...
    click.echo(f"\nSupported Formats:")
    click.echo(f"  - YAML (import/export)")
    click.echo(f"  - JSON (import/export)")
    ...
    click.echo(f"\nUsage:")
    click.echo(f"  fluid odcs export <contract.yaml>      # FLUID → ODCS")
    click.echo(f"  fluid odcs import <contract.yaml>      # ODCS → FLUID")
    click.echo(f"  fluid odcs validate <contract.yaml>    # Validate ODCS")
...
def register(subparsers) -> None:
    ...
    # odcs export
    export = odcs_sub.add_parser(
        "export",
        help="Export FLUID contract to ODCS format"
    )
    ...
    # odcs import
    import_cmd = odcs_sub.add_parser(
        "import",
        help="Import ODCS contract to FLUID format"
    )
    import_cmd.add_argument("odcs_file", help="Path to ODCS contract file")
    import_cmd.add_argument("--output", "-o", help="Output file path")
    import_cmd.add_argument("--format", "-f", choices=["yaml", "json"], default="yaml", help="Output format")
    ...
    # odcs info
    info = odcs_sub.add_parser(
        "info",
        help="Show ODCS provider information"
    )
