...
from .security import SecurityContext, set_security_context, ProductionLogger
from .performance import (
    optimize_startup, profile_command, get_performance_stats, 
    run_health_checks, MemoryMonitor
)
from .help_formatter import print_main_help, print_first_run_help, print_command_help, RICH_AVAILABLE as HELP_RICH_AVAILABLE
...
For more information, visit: https://github.com/agentics-rising/fluid-forge-cli
    ...
    p.add_argument(
        "--config-dir",
        help="Custom configuration directory (default: ~/.fluid)"
    )
    p.add_argument(
        "--no-color",
        action="store_true",
    ...
    p.add_argument(
        "--version",
        action="version",
        version=f"FLUID Forge CLI v{_ver}"
    )
    ...
    prod_group.add_argument(
        "--profile",
        action="store_true",
        help="Enable performance profiling for commands"
    )
    prod_group.add_argument(
        "--health-check",
        action="store_true",
        help="Run operational health checks and exit"
    )
    prod_group.add_argument(
        "--stats",
        action="store_true",
    ...
    prod_group.add_argument(
        "--safe-mode",
        action="store_true",
        help="Enable enhanced security validations"
    )
    prod_group.add_argument(
        "--debug",
        action="store_true",
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    try:
        ...
        if getattr(args, 'debug', False) or os.getenv('FLUID_LOG_LEVEL', '').upper() == 'DEBUG':
            cli.logger.log_safe("info", "CLI starting", extra={
        ...
        try:
            ...
            if profiling_enabled or getattr(args, 'debug', False):
                ...
                cli.logger.log_safe("info", "Command completed", extra={
                    "cmd": getattr(args, "cmd", None),
            ...
            cli.logger.log_safe("error", "CLI command error", extra={
                "cmd": getattr(args, "cmd", None),
                "event": e.event,
            ...
            cli.logger.log_safe("info", "CLI interrupted by user", extra={
                "cmd": getattr(args, "cmd", None)
            })
            return 1
            ...
            cli.logger.log_safe("info", "CLI system exit", extra={
                "cmd": getattr(args, "cmd", None), 
                "code": e.code
            })
            raise
            ...
            if ProviderError is not None and isinstance(e, ProviderError):
                ...
                cli.logger.log_safe("error", "Provider error", extra={
                    "cmd": getattr(args, "cmd", None),
                    "error": str(e),
                    "type": type(e).__name__
                })
                context.console.print("\n[yellow]💡 Check provider configuration and credentials.[/yellow]")
                context.console.print("[yellow]Run 'fluid doctor' for diagnostics or 'fluid providers' to list available providers.[/yellow]")
                return 1
            
            # Handle unexpected errors with enhanced context
            context.console.print(f"[red]❌ Unexpected error: {e}[/red]")
            cli.logger.log_safe("error", "CLI unhandled exception", extra={
...
def _setup_enhanced_logging(level: str, log_file: Optional[str], no_color: bool = False, debug: bool = False) -> ProductionLogger:
    ...
    logging.basicConfig(
        level=numeric,
        format='%(message)s' if not log_file else None,
        handlers=[]
    )
...
def _validate_global_args(args: argparse.Namespace, logger: Optional[ProductionLogger] = None) -> None:
    ...
    if hasattr(args, 'provider') and args.provider:
        ...
        if args.provider == 'local' and not getattr(args, 'debug', False):
            ...
            if getattr(args, 'debug', False) or os.getenv('FLUID_LOG_LEVEL', '').upper() == 'DEBUG':
                warnings.append("Local provider detected - ensure this is intended for production use")
    ...
    if not os.getenv('FLUID_PROVIDER') and not getattr(args, 'provider', None):
        ...
        if getattr(args, 'debug', False) or os.getenv('FLUID_LOG_LEVEL', '').upper() == 'DEBUG':
            warnings.append("💡 Tip: Set FLUID_PROVIDER environment variable to avoid specifying --provider each time")
