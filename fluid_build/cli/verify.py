...
Use Cases:
  ...
  - Pre-deployment checks: Verify before apply
    ...
    p.add_argument(
        "contract",
        help="Path to FLUID contract YAML file"
    )
    ...
    p.add_argument(
        "--strict",
        action="store_true",
        help="Exit with error code if any mismatches found"
    )
...
def verify_bigquery_table(
    ...
    try:
        ...
        has_issues = bool(missing_fields or extra_fields or type_mismatches or mode_mismatches or not region_match)
...
def run(args: argparse.Namespace, logger: logging.Logger) -> int:
    ...
    if args.expose_id:
        if args.expose_id not in exposes_dict:
            raise CLIError(1, "expose_not_found", {"expose_id": args.expose_id, "available": list(exposes_dict.keys())})
    ...
    for expose_name, result in results.items():
        ...
        if structure.get('status') == 'pass':
            ...
            matching = structure.get('matching_fields', [])
            cprint(f"      ❌ FAIL - Schema structure mismatch")
        ...
        if types.get('status') == 'pass':
            cprint(f"      ✅ PASS - All field types match specification")
        else:
            cprint(f"      ❌ FAIL - Type mismatches detected ({len(type_mismatches)})")
            if args.show_diffs:
                for mismatch in type_mismatches:
                    cprint(f"         ≠ {mismatch['field']}: expected {mismatch['expected']}, found {mismatch['actual']}")
        
        # Dimension 3: Constraints
        cprint(f"\n   🔍 Dimension 3: Constraints (nullable/required)")
        mode_mismatches = constraints.get('mismatches', [])
        if constraints.get('status') == 'pass':
            cprint(f"      ✅ PASS - All field constraints match specification")
        else:
