...
class GoogleCloudAuthProvider(AuthProvider):
    ...
    async def check_auth(self) -> AuthResult:
        ...
        try:
            ...
            try:
                result = self._run_command(["gcloud", "auth", "application-default", "print-access-token"], capture_output=True)
                if result.returncode == 0:
                    # Get user info
                    try:
                        account_result = self._run_command(["gcloud", "config", "get-value", "account"], capture_output=True)
                        account = account_result.stdout.strip() if account_result.stdout else "unknown"
                        
                        project_result = self._run_command(["gcloud", "config", "get-value", "project"], capture_output=True)
                        project = project_result.stdout.strip() if project_result.stdout else self.project_id
...
class AWSAuthProvider(AuthProvider):
    ...
    async def check_auth(self) -> AuthResult:
        ...
        try:
            ...
            # Check credentials
            try:
                command = ["aws", "sts", "get-caller-identity", "--profile", self.profile, "--output", "json"]
...
class AzureAuthProvider(AuthProvider):
    ...
    async def check_auth(self) -> AuthResult:
        ...
        try:
            ...
            try:
                result = self._run_command(["az", "account", "show", "--output", "json"], capture_output=True)
...
class DatabricksAuthProvider(AuthProvider):
    ...
    async def login(self, **kwargs) -> AuthResult:
        ...
        try:
            ...
            if self.console:
                with Progress(
                    ...
                    if self.host:
                        ...
                        self._run_command(command + ["--host", self.host], capture_output=False, check=False)
                    else:
            ...
            test_result = self._run_command(["databricks", "workspace", "list", "/"], capture_output=True, check=False)
            ...
            if test_result.returncode == 0:
                ...
                error_msg = test_result.stderr.strip() if test_result.stderr else "Authentication failed"
                return AuthResult(
                    provider=self.name,
                    status=AuthStatus.NOT_AUTHENTICATED,
                    error_message=error_msg
    ...
    async def check_auth(self) -> AuthResult:
        ...
        try:
            ...
            result = self._run_command(["databricks", "workspace", "list", "/"], capture_output=True, check=False)
            
            if result.returncode == 0:
                # Try to get current user info
                try:
                    user_result = self._run_command(["databricks", "current-user", "me"], capture_output=True, check=False)
                    user_info = {}
...
def register(subparsers: argparse._SubParsersAction):
    ...
    p.add_argument("--provider", "-p", 
                   ...
                   choices=["google_cloud", "gcp", "google", "aws", "amazon", "azure", "microsoft", "snowflake", "databricks"])
    ...
    login_parser.add_argument("provider", nargs="?",
                             help="Provider to authenticate with (overrides --provider)")
    ...
    status_parser.add_argument("provider", nargs="?",
                              help="Provider to check (if not specified, checks all)")
    status_parser.set_defaults(func=run)
    
    # Logout command
    logout_parser = sp.add_parser("logout", help="Logout from a provider")
    logout_parser.add_argument("provider", nargs="?",
                              help="Provider to logout from (overrides --provider)")
...
def run(args, logger: logging.Logger) -> int:
    ...
    try:
        ...
        if args.verb == "login":
            if not provider:
                logger.error("❌ Provider required for login. Use: fluid --provider <provider> login")
            ...
            if not provider:
                logger.error("❌ Provider required for logout. Use: fluid --provider <provider> logout")
                return 1
            ...
            logger.info(f"Authentication command not fully implemented for verb: {getattr(args, 'verb', 'unknown')}")
            return 0
...
async def handle_status(provider: Optional[str], auth_manager: AuthManager, logger: logging.Logger) -> int:
    ...
    try:
        ...
        if provider:
            ...
            if console:
                ...
                console.print("=" * 40)
                console.print(f"Status: [{status_color}]{result.status.value.replace('_', ' ').title()}[/{status_color}]")
            ...
            if console:
                ...
                for provider_name in providers:
                    ...
                    if result.user_info:
                        ...
                        details = result.error_message[:40] + "..." if len(result.error_message) > 40 else result.error_message
