...
def register(subparsers: argparse._SubParsersAction) -> None:
    ...
    parser.add_argument('--provider', 
                       choices=['github_actions', 'gitlab_ci', 'azure_devops', 'jenkins', 'bitbucket', 'circleci', 'tekton'],
    ...
    parser.add_argument('--enable-approvals',
                       action='store_true',
                       help='Enable manual approval gates')
    parser.add_argument('--enable-security-scan',
                       action='store_true',
                       default=True,
                       help='Enable security scanning in pipeline')
    parser.add_argument('--enable-marketplace',
                       action='store_true',
                       help='Enable marketplace publishing')
    parser.add_argument('--output-dir',
                       default='.',
                       help='Directory to output pipeline files')
    parser.add_argument('--preview',
                       action='store_true',
                       help='Preview pipeline content without writing files')
...
def run(args: argparse.Namespace, logger: logging.Logger) -> int:
    ...
    try:
        ...
        if args.interactive or not args.provider:
            provider, complexity, environments, enable_approvals, enable_security_scan, enable_marketplace = _interactive_config()
        else:
...
def _interactive_config():
    ...
    if complexity in ['standard', 'advanced', 'enterprise']:
        env_input = input(f"\nUse default environments ({', '.join(environments)})? [Y/n]: ").lower()
    ...
    if complexity in ['advanced', 'enterprise']:
        enable_approvals = input("\nEnable manual approval gates for production? [Y/n]: ").lower() not in ['n', 'no']
    ...
    return provider, complexity, environments, enable_approvals, enable_security_scan, enable_marketplace
