...
def execute_build(
    ...
    if trigger_type == 'manual':
        ...
        cprint(f"   Cron: {cron}")
        cprint(f"   ⚠️  Scheduled execution requires Cloud Composer/Scheduler (paid tier)")
        cprint(f"   💡 For free tier, use trigger.type: manual with iterations")
        cprint(f"{'=' * 80}")
        return 0
    
    else:
        cprint(f"   ❌ Unknown trigger type: {trigger_type}")
        cprint(f"   Supported types: manual, schedule")
        cprint(f"{'=' * 80}")
        return 1
...
def run(args: argparse.Namespace, logger: logging.Logger) -> int:
    ...
    cprint(f"\n{'=' * 80}")
    cprint(f"🚀 FLUID Execute - Build Execution")
    cprint(f"{'=' * 80}")
    cprint(f"Contract: {contract_path}")
    cprint(f"Builds: {len(builds)}")
    if args.dry_run:
        cprint(f"Mode: DRY RUN")
    cprint(f"{'=' * 80}")
