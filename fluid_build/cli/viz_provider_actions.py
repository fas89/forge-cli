    ...
    for i, action in enumerate(actions):
        ...
        if i < len(actions) - 1:
            ...
            if next_id not in dependencies.get(action_id, []):
                ...
                lines.append(f'    color="#CCCCCC",')
                lines.append(f'    label="then",')
                lines.append(f'    fontsize=9,')
                lines.append(f'    fontcolor="#999999"')
                lines.append(f'  ];')
...
def add_provider_actions_to_viz(contract: Dict, logger) -> Optional[Tuple[List[Dict], Dict[str, List[str]]]]:
    """
        ...
        actions_dicts = [
            {
                ...
                'provider': a.provider,
                'resource': a.params.get('table') or a.params.get('bucket') or a.params.get('dataset'),
                'params': a.params,
            }
