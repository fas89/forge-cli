...
class SearchFilters:
    ...
    search_fields: List[str] = field(default_factory=lambda: ['name', 'description', 'tags'])  # Fields to search
    
    # Faceted search
    facets: Dict[str, List[str]] = field(default_factory=dict)  # e.g., {'domain': ['finance', 'marketing']}
...
class AdvancedSearchEngine:
    ...
    def calculate_relevance_score(self, product: DataProductMetadata, filters: SearchFilters) -> float:
    ...
    def apply_advanced_filters(self, products: List[DataProductMetadata], filters: SearchFilters) -> List[DataProductMetadata]:
            ...
            if filters.facets:
                ...
                for facet_field, facet_values in filters.facets.items():
                    if facet_field == 'domain' and product.domain not in facet_values:
                        ...
                        skip_product = True
                        break
                    elif facet_field == 'tags' and not any(tag in product.tags for tag in facet_values):
                        skip_product = True
                        break
    ...
    def rank_and_sort_products(self, products: List[DataProductMetadata], filters: SearchFilters) -> List[DataProductMetadata]:
    ...
    def save_search(self, filters: SearchFilters) -> bool:
        ...
        saved_filters = SearchFilters(**{
            ...
            if k not in ['save_search', 'search_name']
        })
    ...
    def generate_search_suggestions(self, products: List[DataProductMetadata], query: str) -> List[str]:
...
async def retry_with_backoff(func, max_retries: int = 3, base_delay: float = 1.0, max_delay: float = 60.0, backoff_factor: float = 2.0):
...
class BaseCatalogConnector:
    ...
    async def _ensure_healthy_connection(self) -> None:
        ...
        if (self.last_health_check is None or 
            ...
            if not await self._health_check():
                self.logger.warning(f"Health check failed for {self.catalog_type}, attempting reconnection")
                await self.connect()
    ...
    def _apply_filters(self, products: List[DataProductMetadata], filters: SearchFilters) -> List[DataProductMetadata]:
        ...
        for product in products:
            ...
            if filters.status and product.status != filters.status:
                continue
            if filters.min_quality_score and (not product.quality_score or product.quality_score < filters.min_quality_score):
                continue
            if filters.text_query:
                query_lower = filters.text_query.lower()
                if not any(query_lower in text.lower() for text in [
                    product.name, product.description, product.domain, 
                    ' '.join(product.tags)
                ]):
                    continue
...
class AWSGlueDataCatalogConnector(BaseCatalogConnector):
    ...
    async def _get_catalog_stats_impl(self) -> Dict[str, Any]:
        ...
        return {
            "total_products": 25,
            "avg_quality": 0.88,
            "last_updated": "2024-10-15T09:30:00Z"
        }
...
class AzurePurviewConnector(BaseCatalogConnector):
    ...
    async def _get_catalog_stats_impl(self) -> Dict[str, Any]:
        ...
        return {
            "total_products": 18,
            "avg_quality": 0.83,
            "last_updated": "2024-10-15T08:45:00Z"
        }
...
class DataHubConnector(BaseCatalogConnector):
    ...
    async def _get_catalog_stats_impl(self) -> Dict[str, Any]:
        ...
        return {
            "total_products": 35,
            "avg_quality": 0.91,
            "last_updated": "2024-10-15T11:15:00Z"
        }
    ...
    async def _connect_impl(self) -> bool:
        ...
        try:
            ...
            if not username or not password:
                self.logger.error("Apache Atlas credentials required. Set 'username'/'password' in config or ATLAS_USERNAME/ATLAS_PASSWORD env vars.")
                return False
...
class ConfluentSchemaRegistryConnector(BaseCatalogConnector):
    ...
    async def _get_catalog_stats_impl(self) -> Dict[str, Any]:
        ...
        return {
            "total_products": 28,
            "avg_quality": 0.94,
            "last_updated": "2024-10-15T12:30:00Z"
        }
...
class CollibraConnector(BaseCatalogConnector):
    ...
    async def _get_catalog_stats_impl(self) -> Dict[str, Any]:
        ...
        return {
            "total_products": 67,
            "avg_quality": 0.95,
            "last_updated": "2024-10-15T08:15:00Z"
        }
...
class AlationConnector(BaseCatalogConnector):
    ...
    async def _get_catalog_stats_impl(self) -> Dict[str, Any]:
        ...
        return {
            "total_products": 89,
            "avg_quality": 0.93,
            "last_updated": "2024-10-15T07:45:00Z"
        }
...
class CustomRestApiConnector(BaseCatalogConnector):
    ...
    async def _connect_impl(self) -> bool:
        ...
        try:
            ...
            self.logger.info(f"Connecting to Custom REST API (server: {base_url}, auth: {auth_type})")
    ...
    async def _get_catalog_stats_impl(self) -> Dict[str, Any]:
        ...
        return {
            "total_products": 15,
            "avg_quality": 0.86,
            "last_updated": "2024-10-15T11:00:00Z"
        }
                ...
                for item in data.get('items', []):
                    try:
                        ...
                        product = DataProductMetadata(
                            ...
                            owner=item.get('owner', {}).get('name', 'unknown') if isinstance(item.get('owner'), dict) else item.get('owner', 'unknown'),
                            layer=layer,
                            status=status,
                            tags=item.get('tags', []),
...
class MetricsCollector:
    ...
    def update_connector_health(self, catalog_type: str, is_healthy: bool, response_time: float = None):
    ...
    def update_circuit_breaker_stats(self, catalog_type: str, state: str, failure_count: int, success_count: int):
        ...
        for name, connector in self.connectors.items():
            try:
                ...
                health_report['connectors'][name] = {
                    ...
                    'response_time': response_time,
                    'circuit_breaker_state': connector.circuit_breaker.state if hasattr(connector, 'circuit_breaker') else 'unknown'
                }
...
class MarketCache:
    ...
    def set(self, method: str, catalog_type: str, filters: SearchFilters, data: Any, ttl_minutes: Optional[int] = None) -> None:
...
class MarketDiscoveryEngine:
    ...
    async def advanced_search(self, filters: SearchFilters) -> SearchResult:
        ...
        if filters.text_query and len(filtered_products) < 5:  # Only suggest if few results
            suggestions = advanced_search_engine.generate_search_suggestions(all_products, filters.text_query)
    ...
    async def search_all_catalogs(self, filters: SearchFilters) -> Dict[str, List[DataProductMetadata]]:
                ...
                for catalog_name, connector in self.connectors.items():
                    ...
                    try:
                        ...
                        if self.cache_enabled and self.cache:
                            cached_result = self.cache.get('search_data_products', catalog_name, filters)
    ...
    async def _search_with_timeout(self, connector: BaseCatalogConnector, filters: SearchFilters) -> List[DataProductMetadata]:
        """Search with timeout and error handling"""
        timeout_seconds = self.config.get('defaults', {}).get('timeout_seconds', 30)
        
        try:
            return await asyncio.wait_for(
                connector.search_data_products(filters),
    ...
    def aggregate_results(self, catalog_results: Dict[str, List[DataProductMetadata]]) -> List[DataProductMetadata]:
...
def format_table_output(products: List[DataProductMetadata], console: Optional[Console] = None) -> None:
    ...
    if console and RICH_AVAILABLE:
        ...
        for product in products:
            ...
            quality_color = "green" if product.quality_score and product.quality_score >= 0.9 else "yellow" if product.quality_score and product.quality_score >= 0.7 else "red"
        ...
        cprint(f"{'ID':<25} {'Name':<30} {'Domain':<15} {'Layer':<12} {'Quality':<8} {'Source':<20}")
        cprint("-" * 120)
        
        for product in products:
            quality_str = f"{product.quality_score:.2f}" if product.quality_score else "N/A"
            cprint(f"{product.id:<25} {product.name[:29]:<30} {product.domain:<15} {product.layer.value:<12} {quality_str:<8} {product.catalog_source:<20}")
...
they need for their data products.
    ...
    search_group.add_argument(
        "--search", "-s",
        help="Text search across product names, descriptions, and tags"
    )
    search_group.add_argument(
        "--domain", "-d",
        action="append",
        help="Filter by domain(s) (can be used multiple times)"
    )
    search_group.add_argument(
        "--owner", "-o",
        action="append", 
        help="Filter by owner(s) (can be used multiple times)"
    )
    search_group.add_argument(
        "--layer", "-l",
    ...
    filter_group.add_argument(
        "--created-after",
        help="Show products created after date (YYYY-MM-DD)"
    )
    filter_group.add_argument(
        "--created-before", 
        help="Show products created before date (YYYY-MM-DD)"
    )
    ...
    catalog_group.add_argument(
        "--catalogs",
        help="Comma-separated list of catalogs to search (default: all configured)"
    )
    catalog_group.add_argument(
        "--list-catalogs",
        action="store_true",
    ...
    output_group.add_argument(
        ...
        type=int,
        default=50,
        help="Maximum number of results per catalog (default: 50)"
    )
    output_group.add_argument(
        "--offset",
        type=int,
    ...
    detail_group.add_argument(
        "--product-id",
        help="Get detailed information about a specific product"
    )
    detail_group.add_argument(
        "--detailed",
        action="store_true",
    ...
    info_group.add_argument(
        "--marketplace-stats",
        action="store_true",
        help="Show marketplace statistics and summary"
    )
    info_group.add_argument(
        "--config-template",
        action="store_true",
...
def _load_env_config() -> Dict[str, Any]:
    ...
    if os.getenv('FLUID_MARKET_MIN_QUALITY'):
        config.setdefault('defaults', {})['min_quality_score'] = float(os.getenv('FLUID_MARKET_MIN_QUALITY'))
    
    if os.getenv('FLUID_MARKET_TIMEOUT'):
        config.setdefault('defaults', {})['timeout_seconds'] = int(os.getenv('FLUID_MARKET_TIMEOUT'))
...
def generate_output(products: List[DataProductMetadata], args, console: Optional[Console], logger: logging.Logger) -> int:
    ...
    try:
        ...
        if engine.console and RICH_AVAILABLE:
            ...
            if slow_queries.get('count', 0) > 0:
                ...
                for query in recent:
                    engine.console.print(f"  {query['catalog_type']}: {query['latency']:.2f}s at {query['timestamp']}")
        ...
        try:
            while True:
                ...
                if engine.console and RICH_AVAILABLE:
                    ...
                    engine.console.print(f"[bold blue]🔍 Market Health Monitor - {current_time}[/bold blue]")
                    ...
                    engine.console.print(f"Overall Status: [{status_color}]{status.upper()}[/{status_color}]")
                    engine.console.print(f"Health Score: {health_status['overall_health_score']:.1%}")
                    
                    # Connector status
                    engine.console.print("\n🔌 Connector Status:")
                    for name, connector_health in health_status['connectors'].items():
                        connector_status = connector_health['status']
                        status_emoji = {'healthy': '✅', 'unhealthy': '❌', 'error': '💥'}.get(connector_status, '❓')
                    ...
                    cprint(f"[{current_time}] Status: {health_status['status']} | Score: {health_status['overall_health_score']:.1%}")
...
async def handle_search_suggestions(query: str, engine: MarketDiscoveryEngine, logger: logging.Logger) -> int:
        ...
        if engine.console and RICH_AVAILABLE:
            ...
            for catalog_name, connector in engine.connectors.items():
                try:
                    ...
                    engine.console.print(f"\n🏪 {catalog_name}")
                    engine.console.print(f"  📦 Total Products: {stats.get('total_products', 'N/A')}")
...
async def handle_product_details(engine: MarketDiscoveryEngine, product_id: str, args, logger: logging.Logger) -> int:
