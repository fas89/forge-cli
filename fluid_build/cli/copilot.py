...
class FluidAICopilot:
    ...
    def auto_recover_from_error(self, error_context: Dict[str, Any]) -> Optional[List[str]]:
        ...
        recovery_actions = {
            ...
            "contract_not_found": ["fluid forge --template quickstart", "ls *.yaml *.yml *.json"],
            "validation_error": [
...
def register(subparsers: argparse._SubParsersAction):
    ...
    p = subparsers.add_parser(COMMAND, help="🧠 AI-powered assistant for FLUID development")

    copilot_subparsers = p.add_subparsers(dest="copilot_action", help="Copilot actions")

    # Analyze command
    analyze_parser = copilot_subparsers.add_parser(
        "analyze", help="Analyze current context and suggest improvements"
    )
    ...
    suggest_parser.add_argument(
        "--success", action="store_true", help="Whether last command succeeded"
    )

    # Interactive mode
    _interactive_parser = copilot_subparsers.add_parser(
        "interactive", help="Start interactive AI assistant"
    )  # noqa: F841
...
async def run(args, logger: logging.Logger) -> int:
    ...
    try:
        ...
        if args.copilot_action == "analyze":
            ...
            copilot.console.print(
                "[red]❌ Unknown copilot action. Use 'fluid copilot --help' for available options.[/red]"
            )
            return 1
...
async def handle_analyze(args, copilot: FluidAICopilot, logger: logging.Logger) -> int:
    ...
    if suggestions:
        ...
        for i, suggestion in enumerate(suggestions, 1):
            ...
            panel_content.append("💡 Action: ", style="bold yellow")
            panel_content.append(f"{suggestion.action}\n", style="cyan")
            panel_content.append("🤔 Why: ", style="bold green")
