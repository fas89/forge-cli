...
def register(subparsers: argparse._SubParsersAction):
    ...
    search_parser = marketplace_subparsers.add_parser(
        'search',
        help='Search marketplace blueprints'
    )
    search_parser.add_argument('query', nargs='?', help='Search query (keywords, categories)')
    search_parser.add_argument('--category', help='Filter by category (analytics, streaming, ml, governance)')
    search_parser.add_argument('--tags', help='Filter by tags (comma-separated)')
    search_parser.add_argument('--maturity', help='Filter by maturity (experimental, stable, production)')
    search_parser.add_argument('--state', default='published', help='Filter by state (published, draft, deprecated)')
    search_parser.add_argument('--sort', default='downloads', help='Sort by (downloads, updated, name)')
    search_parser.add_argument('--limit', type=int, default=20, help='Max results')
    
    # Get blueprint info
    info_parser = marketplace_subparsers.add_parser(
        'info',
        help='Show detailed blueprint information'
    )
    ...
    instantiate_parser = marketplace_subparsers.add_parser(
        'instantiate',
        help='Generate FLUID contract from blueprint'
    )
    ...
    instantiate_parser.add_argument('--interactive', '-i', action='store_true', help='Interactive parameter wizard')
    instantiate_parser.add_argument('--output', '-o', help='Output file for generated contract')
    instantiate_parser.add_argument('--validate', action='store_true', default=True, help='Validate generated contract')
    instantiate_parser.add_argument('--submit', action='store_true', help='Submit contract to FLUID immediately')
    
    # List categories
    _categories_parser = marketplace_subparsers.add_parser(  # noqa: F841
        'categories',
        help='List available blueprint categories'
    )
...
def get_api_url(logger: Optional[logging.Logger] = None) -> str:
    ...
    if cc.url:
        console.print(f"[yellow]⚠️  Command Center at {cc.url} is unavailable or marketplace feature not enabled[/yellow]")
        ...
        if fallback_url:
            if fallback_strategy == 'local':
                ...
                if local_path.exists():
                    console.print(f"[yellow]⚠️  Using local blueprint cache: {fallback_url}[/yellow]")
    ...
    console.print("     [cyan]export FLUID_PUBLIC_REGISTRY=https://blueprints.fluid.io/api/v1[/cyan]")
    ...
    try:
        ...
        for param in params:
            params_table.add_row(
                ...
                param.get('description', '')[:50] + '...' if len(param.get('description', '')) > 50 else param.get('description', '')
            )
...
def instantiate_blueprint(args, logger: logging.Logger, api_url: str) -> int:
    ...
    if args.params:
        ...
        # Interactive wizard
        console.print(Panel(f"[bold]Parameter Wizard[/bold]\nFill in parameters for {bp['name']}", title="🧙"))
        params = bp.get('spec', {}).get('parameters', bp.get('parameters', []))
    ...
    try:
        response = requests.post(
            f"{api_url}/{args.blueprint_id}/instantiate",
            json=payload
        )
        response.raise_for_status()
...
def interactive_parameter_wizard(parameters: list) -> Dict[str, Any]:
    ...
    for param in parameters:
        ...
        if enum:
            ...
            value = Prompt.ask("  Value", choices=[str(v) for v in enum], default=str(default) if default else None)
