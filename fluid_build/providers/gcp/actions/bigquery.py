    ...
    try:
        ...
        try:
            ...
            if schema:
                ...
                has_policy_tags = any(
                    field.policy_tags and field.policy_tags.names 
            ...
            if contract:
                ...
                if masking_rules:
                    ...
                    _apply_data_masking(project, location, normalized_dataset, normalized_table, masking_rules)
                ...
                if column_restrictions:
                    ...
                    _apply_column_restrictions(project, normalized_dataset, normalized_table, schema, column_restrictions)
...
def _convert_schema_to_bq(schema: List[Dict[str, Any]], project: str = None, location: str = None, 
                         taxonomy_uris: Dict[str, Dict[str, str]] = None) -> List:
    """
    ...
    for field in schema:
        ...
        nested_fields = _convert_schema_to_bq(fields, project, location, taxonomy_uris) if fields else ()
...
def _is_schema_different(existing_schema: List, new_schema: List) -> bool:
    ...
    for name, new_field in new_fields.items():
        ...
        if (existing_field.field_type != new_field.field_type or
            existing_field.mode != new_field.mode or
...
def _is_additive_schema_change(existing_schema: List, new_schema: List) -> bool:
    ...
    for new_field in new_schema:
        if new_field.name in existing_fields:
            ...
            if (not types_are_equivalent(existing_field.field_type, new_field.field_type) or
                existing_field.mode != new_field.mode):
...
def _ensure_policy_tags(project: str, location: str, schema: List[Dict[str, Any]]) -> Dict[str, Dict[str, str]]:
    """
    ...
    try:
        ...
        warning("google-cloud-datacatalog not installed, policy tags will be documented in descriptions only")
        return {}
    ...
    try:
        ...
        cprint(f"   Using Data Catalog location: {datacatalog_location} (from BigQuery location: {location})")
        ...
        for taxonomy_name, tags in taxonomies.items():
            try:
                ...
                if taxonomy_name in existing_taxonomies:
                    ...
                    taxonomy = dc_client.create_taxonomy(
                        parent=parent,
                        taxonomy=taxonomy
                    )
                    ...
                    try:
                        ...
                        if tag_name in existing_tags:
                            ...
                            policy_tag = datacatalog_v1.PolicyTag(
                                ...
                                description=f"FLUID policy tag: {tag_name}"
                            )
...
def _apply_data_masking(project: str, location: str, dataset_id: str, table_id: str, 
                       masking_rules: List[Dict[str, Any]]) -> bool:
    """
    ...
    try:
        ...
        warning("google-cloud-bigquery-datapolicies not installed, masking policies will not be created")
            ...
            try:
                ...
                try:
                    ...
                    if "already exists" in str(create_error).lower():
                        ...
                        cprint(f"   ⚠️  Failed to create masking policy for {column}: {create_error}")
...
def _apply_column_restrictions(project: str, dataset_id: str, table_id: str,
                               schema: List[Dict[str, Any]], 
                               restrictions: List[Dict[str, Any]]) -> bool:
    """
    ...
    try:
        ...
        for restriction in restrictions:
            ...
            try:
                ...
                view.description = f"Restricted view for {principal} (excludes: {', '.join(restricted_columns)})"
