...
class GcpProvider(BaseProvider):
    ...
    def __init__(self,
                 ...
                 region: Optional[str] = "europe-west3",
                 logger=None) -> None:
        super().__init__(project=project, region=region, logger=logger)
        self._bq_client = None  # Lazy-load to avoid auth at import
        self._log_info("provider_init", {"provider": "gcp", "project": self.project, "region": self.region})
    ...
    def _extract_governance_labels(self, contract: Dict[str, Any], expose: Dict[str, Any] = None) -> Dict[str, str]:
        """
        ...
        for key, value in contract_labels.items():
            ...
            safe_value = str(value).lower().replace(" ", "_").replace(".", "_")[:63]  # 63 char limit
    ...
    def plan(self, contract: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        for expo in contract.get("exposes", []):
            ...
            if fmt == "bigquery_table":
                ...
                self._log_info("governance_labels_extracted", {
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    self._log_info("dataset_created", {
                        "project": project,
                        "dataset": dataset_id,
                    ...
                    dataset = client.get_dataset(dataset_ref)
                    self._log_info("dataset_exists_after_conflict", {
                        "project": project,
                        "dataset": dataset_id,
                        "location": dataset.location
                    })
    ...
    def _ensure_table(self, a: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                if not schema_spec:
                    self._log_info("table_skipped_no_schema", {
                        "project": project,
                        "dataset": dataset_id,
                        "table": table_id,
                ...
                try:
                    ...
                    # Race condition: created between get and create
                    table = client.get_table(table_ref)
                    self._log_info("table_exists_after_conflict", {
                        "project": project,
                        "dataset": dataset_id,
                        "table": table_id,
                        "num_rows": table.num_rows,
    ...
    def _load_csv(self, a: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            raise ProviderError(f"Failed to load CSV {csv_path} into {project}.{dataset_id}.{table_id}: {e}")
