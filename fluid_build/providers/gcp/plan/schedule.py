...
def _plan_scheduled_execution(
    ...
    if composer_env:
        ...
        actions.extend(_plan_composer_dag(
            trigger, contract, project, composer_location, composer_env, logger
        ))
    else:
        # Use Cloud Scheduler + Cloud Run for serverless execution
        actions.extend(_plan_scheduler_job(
...
def _plan_composer_dag(
    ...
    actions.append({
        ...
        "default_args": {
            "owner": contract.get("metadata", {}).get("owner", {}).get("email", "fluid@company.com"),
...
def _plan_event_driven_execution(
    ...
    actions.append({
        ...
        } if properties.get("dead_letter_topic") else None,
