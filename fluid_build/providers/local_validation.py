...
class LocalValidationProvider(ValidationProvider):
    ...
    def _get_duckdb(self):
        ...
        if self._duckdb is None:
            try:
                ...
                raise RuntimeError(
                    "duckdb is required for local validation. "
    ...
    def get_resource_schema(self, resource_spec: Dict[str, Any]) -> Optional[ResourceSchema]:
        ...
        try:
            ...
            raise Exception("Error reading local file: {}".format(str(e))) from e
    ...
    def validate_resource(
        ...
        if actual_schema is None:
            ...
            issues.append(ValidationIssue(
                ...
                message="File '{}' does not exist".format(file_path),
        ...
        if actual_schema.row_count == 0:
            issues.append(ValidationIssue(
                ...
                message="File '{}' has no rows".format(actual_schema.fully_qualified_name),
                path="exposes[].binding.location",
                expected="> 0 rows",
                actual="0 rows",
        ...
        if "row_count_min" in sla and actual_schema.row_count is not None:
            ...
            if actual_schema.row_count < min_rows:
                issues.append(ValidationIssue(
                    ...
                    message="File has {} rows, below minimum of {}".format(
    ...
    def run_quality_checks(
        ...
        if not path.exists():
            return [ValidationIssue(
                severity="warning", category="quality",
                message="Cannot run quality checks: file '{}' not found".format(file_path),
                path="contract.dq.rules",
            )]
        ext = path.suffix.lower()
        if ext in (".parquet", ".pq"):
            ...
            return [ValidationIssue(
                severity="warning", category="quality",
                message="Unsupported file format for quality checks: {}".format(ext),
                path="contract.dq.rules",
            )]
        abs_path = str(path.resolve())
