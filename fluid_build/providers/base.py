...
try:
    ...
    @dataclass
    class PlanAction:
        action_type: str; op: str; resource_id: str
        params: Dict[str, Any] = field(default_factory=dict)

    @dataclass
    class ApplyResult:
        provider: str; applied: int; failed: int
        duration_sec: float; timestamp: str
        results: List[Dict[str, Any]] = field(default_factory=list)
        def to_json(self) -> str:
            return json.dumps({"provider": self.provider, "applied": self.applied,
                "failed": self.failed, "duration_sec": self.duration_sec,
                "timestamp": self.timestamp, "results": self.results}, indent=None)
    ...
    class ProviderInternalError(RuntimeError): pass

    @dataclass
    class ProviderMetadata:
        name: str; display_name: str = ""; description: str = ""
        version: str = "0.0.0"; sdk_version: str = "0.0.0"; author: str = "Unknown"
        url: Optional[str] = None; license: Optional[str] = None
        ...
        def to_dict(self) -> Dict[str, Any]:
            return {k: getattr(self, k) for k in
                    ("name","display_name","description","version","sdk_version",
                     "author","url","license","supported_platforms","tags")}

    class ProviderCapabilities:
        def __init__(self, planning=True, apply=True, render=False, graph=False, auth=False, **extra):
            self._d = {"planning": planning, "apply": apply, "render": render, "graph": graph, "auth": auth}
    ...
    class BaseProvider(ABC):
        ...
        def __init__(self, *, project=None, region=None, logger=None, **kwargs):
            self.project = project; self.region = region
            self.logger = _mk_logger_fb(self.__class__.__module__, logger)
            self.extra: Dict[str, Any] = dict(kwargs)
        def capabilities(self) -> Mapping[str, bool]: return ProviderCapabilities()
        @classmethod
        def get_provider_info(cls) -> "ProviderMetadata": return ProviderMetadata(name=cls.name)
        @abstractmethod
        ...
        def require(self, cond, msg):
            if not cond: self.logger.error("precondition_failed: %s", msg); raise ProviderError(msg)
        def debug_kv(self, **kv):
            try: self.logger.debug(json.dumps(kv))
            except Exception: self.logger.debug(str(kv))
        def info_kv(self, **kv):
            try: self.logger.info(json.dumps(kv))
            except Exception: self.logger.info(str(kv))
        def warn_kv(self, **kv):
            try: self.logger.warning(json.dumps(kv))
            except Exception: self.logger.warning(str(kv))
        def err_kv(self, **kv):
            try: self.logger.error(json.dumps(kv))
            except Exception: self.logger.error(str(kv))

    # -- Hook fallbacks (kept in sync with fluid_provider_sdk.hooks) ------

    @dataclass
    class CostEstimate:
        currency: str = "USD"; monthly: float = 0.0; one_time: float = 0.0
        breakdown: List[Dict[str, Any]] = field(default_factory=list); notes: str = ""
        def total(self) -> float: return self.monthly + self.one_time
        def to_dict(self) -> Dict[str, Any]:
            return {"currency": self.currency, "monthly": self.monthly,
                    "one_time": self.one_time, "total": self.total(),
                    "breakdown": self.breakdown, "notes": self.notes}

    class ProviderHookSpec:
        def pre_plan(self, contract): return contract
        def post_plan(self, actions): return actions
        def pre_apply(self, actions): return actions
        def post_apply(self, result): pass
        def on_error(self, error, context): pass
        def estimate_cost(self, actions): return None
        def validate_sovereignty(self, contract): return []
...
def register_provider(name: str, cls: Type[BaseProvider], logger: Optional[logging.Logger] = None) -> None:
...
def list_providers(logger: Optional[logging.Logger] = None, *, lazy_discover: bool = True) -> List[str]:
...
def get_provider(name: str, logger: Optional[logging.Logger] = None, *, lazy_discover: bool = True) -> Type[BaseProvider]:
...
class _RegistryProxy(dict):
    ...
    def __getitem__(self, key): return self._reg()[key]
    def __contains__(self, key): return key in self._reg()
    def __iter__(self): return iter(self._reg())
    def __len__(self): return len(self._reg())
    def get(self, key, default=None): return self._reg().get(key, default)
    def keys(self): return self._reg().keys()
    def values(self): return self._reg().values()
    def items(self): return self._reg().items()
    def __repr__(self): return repr(self._reg())
...
class _ErrorsProxy(list):
    ...
    def __getitem__(self, idx): return self._errs()[idx]
    def __iter__(self): return iter(self._errs())
    def __len__(self): return len(self._errs())
    def __repr__(self): return repr(self._errs())
