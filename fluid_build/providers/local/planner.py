...
try:
    ...
    def get_primary_build(contract): return contract.get("build") or contract.get("builds", [{}])[0]
    def get_builds(contract): return contract.get("builds") or ([contract.get("build")] if contract.get("build") else [])
    def get_consumes(contract): return contract.get("consumes", [])
    def get_exposes(contract): return contract.get("exposes", [])
    def get_expose_id(exp): return exp.get("id", "output")
    def get_expose_location(exp): return exp.get("location") or {}
...
def plan_actions(
    ...
    if consume_specs is not None:
        ...
        for consume in (consumes or []):
            ...
            path = (
                consume.get("path") or
                consume.get("location", {}).get("path") if isinstance(consume.get("location"), dict) else consume.get("path")
            )
            _consume_iter.append({
                "_id": cid,
                "_path": path,
    ...
    for build_idx, build in enumerate(builds):
        ...
        for inp in param_inputs:
            if isinstance(inp, dict):
                ...
                if name and path:
                    input_specs.append({"table": name, "path": path, "format": inp.get("format", "csv")})
                elif name:
    ...
    for _ei in _expose_iter:
        ...
        source_table = _determine_source_table(_ei["_raw"] if isinstance(_ei.get("_raw"), dict) else {}, builds, loaded_tables, log)
...
def _topological_sort(
    ...
    resource_to_action = {
        action.get("resource_id"): action
        for action in actions
        if action.get("resource_id")
    }
