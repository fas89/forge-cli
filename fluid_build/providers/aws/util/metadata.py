...
class MetadataExtractor:
    ...
    def _extract_policy_tags(self, contract: Dict[str, Any]) -> Dict[str, str]:
        ...
        if policy.get("encryption_required"):
            tags[f"{self.tag_prefix}:encryption_required"] = str(policy["encryption_required"]).lower()
    ...
    def _extract_lineage_tags(self, contract: Dict[str, Any]) -> Dict[str, str]:
        ...
        if dependencies:
            dep_ids = [dep.get("id") or dep.get("name") for dep in dependencies if isinstance(dep, dict)]
