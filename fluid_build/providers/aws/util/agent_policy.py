...
class AgentPolicyExtractor:
    ...
    def extract_tags(
        ...
        if agent_policy.get("auditLevel"):
            tags["fluid:agent_audit_level"] = sanitize_tag_value(
                str(agent_policy["auditLevel"])
            )
        
        # 6. Access tier (if specified)
        if agent_policy.get("accessTier"):
            tags["fluid:agent_access_tier"] = sanitize_tag_value(
                str(agent_policy["accessTier"])
            )
    ...
    def generate_lake_formation_policy(
        ...
        data_filter = {
            ...
            "RowFilter": {
                "FilterExpression": "1=1"  # Default: allow all rows
            },
        ...
        if restricted_columns:
            ...
            data_filter["ColumnWildcard"] = {
                "ExcludedColumnNames": restricted_columns
            }
        
        self.logger.info(f"Generated Lake Formation data filter for agent policy")
        return data_filter
