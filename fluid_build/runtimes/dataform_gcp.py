...
def run_dataform_workspace(
    ...
    *,
    ...
    try:
        ...
        if not project_id:
            raise ProviderError("Could not determine GCP project ID. Set GCLOUD_PROJECT or pass project_id.")
        ...
        operation = client.create_compilation_result(
            parent=parent,
            compilation_result=compilation_request
        )
        ...
        if wait:
            result = _wait_for_compilation(
                client, operation.name, timeout
            )
        ...
        duration = time.time() - start_time
        raise ProviderError(
            f"Dataform deployment failed: {str(e)}"
        ) from e
...
def _stub_dataform_result(workspace: str, start_time: float) -> ApplyResult:
    ...
    """
    duration = time.time() - start_time
    logger.info(f"Dataform workspace {workspace} compiled (stub - install google-cloud-dataform for real deployment)")
