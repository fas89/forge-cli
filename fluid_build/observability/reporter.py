...
class CircuitBreaker:
        ...
        with self._lock:
            ...
            if self.state == CircuitState.HALF_OPEN:
                ...
                if self.success_count >= self.success_threshold:
                    logger.info("Circuit breaker transitioning HALF_OPEN → CLOSED (recovery confirmed)")
                    self.state = CircuitState.CLOSED
                    self.success_count = 0
    ...
    def _on_failure(self, error: Exception):
        ...
        with self._lock:
            ...
            if self.state == CircuitState.HALF_OPEN:
                logger.warning(f"Circuit breaker transitioning HALF_OPEN → OPEN (recovery failed: {error})")
...
class CommandCenterReporter:
    ...
    def send_logs(self, execution_id: str, logs: List[Dict[str, Any]]):
        ...
        payload = {
            "execution_id": execution_id,
            "logs": logs[:1000]  # Enforce max batch size
        }
