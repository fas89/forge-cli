...
def _is_obj(x: Any) -> bool: return isinstance(x, dict)
...
def _check_column(col: Any, path: str, errors: List[str]) -> None:
    ...
    if not _is_obj(col): return
    name = _req(col, "name", errors, f"{path}")
    ctype = _req(col, "type", errors, f"{path}")
    if name is not None: _assert(_is_str(name), f"{path}.name must be string", errors)
    if ctype is not None: _assert(_is_str(ctype), f"{path}.type must be string", errors)

def _check_location(loc: Any, path: str, errors: List[str]) -> None:
    _assert(_is_obj(loc), f"{path} must be object", errors)
    if not _is_obj(loc): return
    fmt = _req(loc, "format", errors, path)
    props = _req(loc, "properties", errors, path)
    if fmt is not None: _assert(_is_str(fmt), f"{path}.format must be string", errors)
    if props is not None: _assert(_is_obj(props), f"{path}.properties must be object", errors)

def _check_expose(ex: Any, idx: int, errors: List[str]) -> None:
    path = f"exposes[{idx}]"
    _assert(_is_obj(ex), f"{path} must be object", errors)
    if not _is_obj(ex): return
    ...
    if _id is not None: _check_id(_id, f"{path}.id", errors)
    if _type is not None: _assert(_is_str(_type), f"{path}.type must be string", errors)
    if _loc is not None: _check_location(_loc, f"{path}.location", errors)
...
def _check_metadata(md: Any, errors: List[str]) -> None:
    ...
    if owner is not None:
        if _is_str(owner):
            ...
            if team is not None: _assert(_is_str(team), "metadata.owner.team must be string", errors)
            email = _opt(owner, "email")
            if email is not None: _check_email(email, "metadata.owner.email", errors)
        else:
...
def _check_build(build: Any, errors: List[str]) -> None:
    ...
    if not _is_obj(build): return
    tr = _req(build, "transformation", errors, "build")
    if tr is None or not _is_obj(tr): return
    ...
    if pattern is not None:
        _assert(pattern in _PATTERNS, f"build.transformation.pattern must be one of {sorted(_PATTERNS)}", errors)
    if engine is not None: _assert(_is_str(engine), "build.transformation.engine must be string", errors)
    if props is not None: _assert(_is_obj(props), "build.transformation.properties must be object", errors)
    ...
    if _is_str(pattern) and _is_obj(props):
        if pattern == "hybrid-reference":
            ...
            if model is not None: _assert(_is_str(model), "build.transformation.properties.model must be string", errors)
            vars_ = _opt(props, "vars")
            if vars_ is not None: _assert(_is_obj(vars_), "build.transformation.properties.vars must be object", errors)
        elif pattern == "declarative":
            # Optional keys but types must be correct if present
            if "from" in props: _assert(_is_str(props["from"]), "declarative.from must be string", errors)
            if "joins" in props:
            ...
            if sql is not None: _assert(_is_str(sql), "embedded-logic.sql must be string", errors)
            lang = _opt(props, "language")
            if lang is not None:
                _assert(lang in {"sql", "flink_sql", "pyspark", "scala"}, "embedded-logic.language invalid", errors)
            ...
            if srcs is not None: _assert(_is_list(srcs), "logical-mapping.sources must be array", errors)
            if steps is not None: _assert(_is_list(steps), "logical-mapping.steps must be array", errors)
...
def validate_contract(contract: Dict[str, Any]) -> Tuple[bool, Optional[str]]:
    ...
    # Types & patterns
    if fv is not None: _check_version(fv, "fluidVersion", errors)
    if k is not None: _assert(_is_str(k), "kind must be string", errors)
    if cid is not None: _check_id(cid, "id", errors)
    if name is not None:
        _assert(_is_str(name), "name must be string", errors)
        if _is_str(name): _assert(bool(_NAME_PATTERN.match(name)), "name must be 1..256 chars", errors)
    if domain is not None: _check_id(domain, "domain", errors)
    if md is not None: _check_metadata(md, errors)

    # consumes (optional)
    consumes = _opt(contract, "consumes")
    if consumes is not None: _check_consumes(consumes, errors)

    # build (optional but common)
    build = _opt(contract, "build")
    if build is not None: _check_build(build, errors)
