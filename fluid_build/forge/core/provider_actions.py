...
class ProviderActionParser:
    ...
    def parse(self, contract: Dict[str, Any]) -> List[ProviderAction]:
        ...
        self.logger.debug(f"No providerActions found, inferring from 0.5.7 patterns (version: {fluid_version})")
    ...
    def get_execution_order(self, actions: List[ProviderAction]) -> List[List[str]]:
        ...
        while remaining:
            ...
            current_level = [
                action_id for action_id in remaining
...
def filter_actions_by_provider(actions: List[ProviderAction], provider: str) -> List[ProviderAction]:
    """Filter actions by provider."""
    return [a for a in actions if a.provider == provider]


def filter_actions_by_type(actions: List[ProviderAction], action_type: ActionType) -> List[ProviderAction]:
