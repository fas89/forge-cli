    ...
    if not os.access(path_obj, os.R_OK):
        raise ValidationError(
            ...
            suggestions=[
                ...
                f"Run: chmod +r {path}" if os.name != 'nt' else f"Run: icacls {path} /grant Users:R"
            ]
        )
...
def validate_directory_exists(path: Union[str, Path], *, description: str = "Directory") -> Path:
    ...
    if not path_obj.exists():
        raise ValidationError(
            ...
            suggestions=[
                f"Create the directory: mkdir -p {path}" if os.name != 'nt' else f"Create the directory: md {path}",
...
def validate_writable_path(path: Union[str, Path], *, description: str = "Output path") -> Path:
    ...
    if not parent.exists():
        raise ValidationError(
            ...
            suggestions=[
                f"Create parent directory: mkdir -p {parent}" if os.name != 'nt' else f"Create parent directory: md {parent}",
    ...
    if not os.access(parent, os.W_OK):
        raise ValidationError(
            ...
            suggestions=[
                ...
                f"Run: chmod +w {parent}" if os.name != 'nt' else f"Run: icacls {parent} /grant Users:W"
            ]
        )
    ...
    if path_obj.suffix.lower() not in valid_extensions:
        raise ValidationError(
            ...
            suggestions=[
                ...
                f"Rename file to have a valid extension"
            ]
        )
    ...
    if not re.match(pattern, value):
        raise ValidationError(
            ...
            suggestions=[
                ...
                f"{field_name} can only contain letters, numbers, underscores" + (", and hyphens" if allow_hyphens else ""),
                f"Example: my_data_product" + (" or my-data-product" if allow_hyphens else "")
            ]
        )
...
def validate_enum(value: str, *, field_name: str, allowed_values: List[str], case_sensitive: bool = False) -> str:
    """
    ...
    try:
        ...
        if not parsed.scheme:
            raise ValidationError(
                ...
                context={"field": field_name, "value": value},
                suggestions=[
                    "Add protocol: https://example.com" if require_https else "Add protocol: http://example.com or https://example.com"
                ]
            )
        ...
        if not parsed.netloc:
            raise ValidationError(
                ...
                context={"field": field_name, "value": value},
                suggestions=[
                    "Provide a valid hostname: https://example.com"
                ]
            )
        ...
        raise ValidationError(
            ...
            context={"field": field_name, "value": value},
            suggestions=[
                "Provide a valid URL: https://example.com"
            ]
        )
...
def validate_int_range(value: int, *, field_name: str, min_value: Optional[int] = None, max_value: Optional[int] = None) -> int:
    """
...
def validate_with_custom(value: Any, *, field_name: str, validator: Callable[[Any], bool], error_message: str) -> Any:
    """
