...
class PolicyEnforcementResult:
    ...
    def is_compliant(self) -> bool:
        ...
        return not any(
            v.severity in [PolicySeverity.CRITICAL, PolicySeverity.ERROR]
...
class SchemaBasedPolicyEngine:
            ...
            for field in schema:
                ...
                if sensitivity in ['pii', 'phi']:
                    if self._has_privacy_protection(expose, field_name):
                        ...
                        self._add_violation(PolicyViolation(
                ...
                if sensitivity == 'cleartext' and field.get('tags', []):
                    if any(tag in ['pii', 'phi', 'sensitive'] for tag in field.get('tags', [])):
                        self._add_violation(PolicyViolation(
                            ...
                            field=field_name,
                            expose_id=expose_id,
                            remediation=f"Change sensitivity to 'pii', 'phi', or apply treatment"
                        ))
                    else:
                        self._add_pass()
    ...
    def _enforce_lifecycle_policies(self) -> None:
        ...
        for expose in self.contract.get('exposes', []):
            ...
            has_sensitive = any(
                field.get('sensitivity') in self.SENSITIVE_TYPES
                for field in schema
            )
